...
LRESULT CALLBACK WndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		for (f=-500;f<1000;f++) {
			...
			SetPixel(hdc, (int)f, y,RGB(0xff,0,0));
			y=(int)(cos(f*3.14/180)*100);
			SetPixel(hdc, (int)f, y,RGB(0,0xff,0));
		    y=(int)(tan(f*3.14/180)*100);
			SetPixel(hdc, (int)f, y,RGB(0,0,0xff));
			y=rand()%100;
			SetPixel(hdc, (int)f, y,RGB(rand()%255,rand()%255,rand()%255));	
		}
		EndPaint(hWnd, &ps);
		return 0;
	}
